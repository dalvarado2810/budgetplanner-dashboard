...
class OnIncomeButtonClickActionProcessor :
    ActionProcessor<Home.State, Home.Action.IncomeButtonClick, Home.Effect>() {
    override fun process(
        action: Home.Action.IncomeButtonClick,
        sideEffect: (Home.Effect) -> Unit
    ): Flow<Mutation<Home.State>> {
        return flowOf { currentState ->
            sideEffect(Home.Effect.NavigateToIncomeDialog)
